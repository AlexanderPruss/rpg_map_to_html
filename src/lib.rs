...
pub fn generate_docs(config: Config) {
    ...
    let map_dimensions = PixelPoint {
        x: map_image.width() as i32,
        y: map_image.height() as i32,
    };
    let map_margin = config
        .map_image
        .image_margins
    ...
    let (cell_map, cached_table_of_contents, cached_cutout_images) = match cached_computations {
        None => (
            config.geometry.compute_cell_map(map_dimensions, map_margin),
            HashMap::new(),
            HashMap::new(),
        ),
        Some(cached) => (
            cached.cell_map,
            cached.table_of_contents_map_images_by_filename,
            cached.cutout_images_by_coordinate,
        ),
    };

    //Image handling
    let (skip_empty_cells, image_handling) = image_handling::image_config::resolve_config(
        &config.map_image.skip_empty_cells,
        &config.image_handling_config,
    );
    let table_of_contents_images =
        image_handling::table_of_contents::save_table_of_contents_map_images_with_cache(
            target_directory,
            &map_image,
    ...
    if cached_cutout_images.len() != cutout_images.len() {
        image_handling::visualize_cell_map::save_cell_map_visualization(
            ...
            &cell_map,
            &map_image,
            &image_handling.cell_outline_color,
        );
        caching::persist_cached_objects(
            target_directory,
            &config,
            &map_image,
            cell_map,
...
pub fn read_input_yes_no_with_default(input: &mut String, default: bool) -> bool {
    ...
    loop {
        ...
        match input.to_lowercase().trim() {
            ...
            "n" => return false,
            _ => {
                println!("Couldn't understand the input '{input}'. Please input Y or N.")
            }
        }
    }
}
...
pub fn read_input_until_valid_option(
    ...
    loop {
        ...
        match options
            .iter()
            .find(|option| option.to_lowercase() == user_input)
        {
            None => {
                println!(
                    "Did not understand the input '{user_input}'. Please input one of [{}] (default: {default}).",
                    options.join(", ")
                )
            }
...
impl PixelBox {
    fn intersects(&self, other: &PixelBox) -> bool {
        let x_overlap = self
            .top_left_corner
            .x
            .clamp(other.top_left_corner.x, other.bottom_right_corner.x)
            == self.top_left_corner.x
            || other
                .top_left_corner
                .x
                .clamp(self.top_left_corner.x, self.bottom_right_corner.x)
                == other.top_left_corner.x;
        let y_overlap = self
            .top_left_corner
            .y
            .clamp(other.top_left_corner.y, other.bottom_right_corner.y)
            == self.top_left_corner.y
            || other
                .top_left_corner
                .y
                .clamp(self.top_left_corner.y, self.bottom_right_corner.y)
                == other.top_left_corner.y;
        x_overlap && y_overlap
    }
}
