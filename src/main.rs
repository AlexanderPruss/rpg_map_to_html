use rpg_map_to_html::{LAST_USED_CONFIG, config, generate_docs, read_input_yes_no_with_default};
...
fn main() {
    ...
    if args.len() == 2 {
        ...
        return generate_docs(config::parse_config(config_path));
    }
...
fn generate_with_existing_config() {
    ...
    generate_docs(config::parse_config(PathBuf::from(LAST_USED_CONFIG)))
}
